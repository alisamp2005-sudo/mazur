...
export const voximplantRouter = router({
  ...
  // ============================================================================
  // Agent Creation (Full Automation)
  // ============================================================================

  /**
   * Get phone numbers for account
   */
  getPhoneNumbers: protectedProcedure
    .input(z.object({ accountId: z.number() }))
    .query(async ({ input, ctx }) => {
      const account = await getVoximplantAccountById(input.accountId);
      if (!account || account.userId !== ctx.user.id) {
        throw new Error("Account not found");
      }
  ...
  /**
   * Create full agent: Application + Scenario + Rule + Phone attachment
   */
  createAgent: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Step 1: Create Application in VoximPlant
      const app = await createApplication(credentials, input.agentName);

      // Step 2: Generate VoxEngine JavaScript code
      const scenarioCode = generateScenarioCode({
        elevenlabsApiKey: input.elevenlabsApiKey,
        elevenlabsAgentId: input.elevenlabsAgentId,
      });
      ...
      await createVoximplantApplication({
        voximplantAccountId: input.accountId,
        voximplantApplicationId: String(app.application_id),
        applicationName: input.agentName,
        elevenlabsApiKey: input.elevenlabsApiKey,
        elevenlabsAgentId: input.elevenlabsAgentId,
        voximplantRuleId: "", // Will be filled manually by user
        phoneNumber: input.phoneNumberId,
        status: "inactive", // User needs to complete setup manually
      });
      ...
      return {
        ...
        instructions: {
          step1: "Application created successfully in VoximPlant",
          step2: "Copy the generated JavaScript code below",
          step3: "Go to VoximPlant Control Panel → Applications → " + app.application_name,
          step4: "Create a new Scenario and paste the code",
          step5: "Create a Routing Rule to handle incoming calls",
          step6: "Attach your phone number to the application",
        },
      };
    }),
});
